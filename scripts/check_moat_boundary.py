...
Rules:
  * **MOAT001** no monorepo import (`lib.`, `services.`, `from AitherOS`). awkno
...
archive) — never 0 for "I could not look".
    ...
    python scripts/check_moat_boundary.py [dist/awkno-*.whl dist/*.tar.gz ...]
...
_KEYSTONES = ("awkno/__init__.py", "awkno/cli.py", "awkno/corpus.py", "awkno/generate.py")
...
def self_test() -> int:
    ...
    def wheel(members: dict) -> Path:
        p = Path(tempfile.mkdtemp()) / "awkno-0.0.0-py3-none-any.whl"
    ...
    check("MOAT001 catches a monorepo import",
          any(f.startswith("MOAT001") for f in
              inspect(wheel({**clean, "awkno/x.py": "from lib.core import X\n"}))), True)

    check("MOAT002 catches a debt id",
          any("debt-ledger row id" in f for f in
              inspect(wheel({**clean, "awkno/x.py": "# see D-0000\n"}))), True)

    check("MOAT002 catches a checker rule id",
          any("checker rule id" in f for f in
              inspect(wheel({**clean, "awkno/x.py": "# MOAT001 says so\n"}))), True)
    ...
    try:
        broken = Path(tempfile.mkdtemp()) / "awkno-0.0.0.tar.gz"
